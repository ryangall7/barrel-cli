...
class Gulper {
  ...
  copy ({type, path}, upload = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (type === 'changed' || type === 'added') {
        ...
        pipe.on('end', e => {
          if (upload) {
            sendToShopify('upload', `/${base}${src}`, e => {
              bs.reload()
              resolve()
            })
          } else {
  ...
  concat (upload = false) {
    return new Promise((resolve, reject) => {
      ...
      pipe.on('end', e => {
        ...
        if (upload) {
          sendToShopify('upload', `/config/settings_schema.json`, () => {
            bs.reload()
            resolve()
          })
        } else {
  ...
  watch () {
    const watcher = gulp.watch('src/**/*', {cwd: config.get('cwd')});
    console.log(watcher);
    watcher.on("change",
      (path, stats) => {
        try {
          console.log("watch event", path, stats);
          this.copy({type:"changed", path: path}, true)
        } catch (e) {
          new Err(e)
        }
      }
   ...
   const configWatcher = gulp.watch('src/config/**', {cwd: config.get('cwd')});
